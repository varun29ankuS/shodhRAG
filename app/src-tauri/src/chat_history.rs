    ...
    pub fn add_message(
        ...
        if let Some(space_id) = space_id {
            ...
            if self.global_session.messages.len() > self.max_messages_per_session {
                let remove_count =
                    self.global_session.messages.len() - self.max_messages_per_session;
    ...
    pub fn get_sessions_summary(&self) -> Vec<HashMap<String, String>> {
        ...
        summaries.push({
            ...
            summary.insert(
                ...
                self.global_session.messages.len().to_string(),
            );
            summary.insert(
                "last_updated".to_string(),
                self.global_session.last_updated.clone(),
            );
            summary
        });
