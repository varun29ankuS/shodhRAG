...
use shodh_rag::llm::{
    ...
    DeviceType, QuantizationType, ModelManager,
};
...
#[tauri::command]
pub async fn browse_tokenizer_file(
    app_handle: tauri::AppHandle,
) -> Result<String, String> {
    use tauri_plugin_dialog::DialogExt;
...
pub fn set_custom_model_path(
    ...
    match extension {
        Some("onnx") => {
            ...
            *state.custom_model_path.lock().unwrap() = Some(path.clone());
            ...
            Ok(format!(
                ...
                model_path,
                if has_data_file { "Found" } else { "Not found (may not be required)" }
            ))
        },
        Some("gguf") => {
            // Store the custom path
        ...
        _ => {
            Err("Invalid model file. Please select a .gguf or .onnx file.".to_string())
        }
    }
}
...
pub async fn initialize_llm_with_custom_path(
    ...
    if let Some(model_path) = custom_model_path {
        ...
        *state.config.lock().unwrap() = config.clone();
        ...
        let mut manager = LLMManager::new_with_paths(config, model_path.clone(), custom_tokenizer_path.clone());
...
pub async fn switch_llm_mode(
        ...
        let api_key = if matches!(api_provider, ApiProvider::Ollama) {
            ...
            match api_key_opt {
                Some(key) if !key.trim().is_empty() => {
                    tracing::info!("Found API key for {:?} (length: {})", api_provider, key.len());
                    key
                },
                _ => {
    ...
    *state.config.lock().unwrap() = config.clone();
    ...
    let model_dir = state.model_dir.as_ref().clone();
    tracing::info!("Attempting to switch to mode: {}, model_dir: {}", mode, model_dir.display());
...
pub async fn llm_generate(
    ...
    tracing::info!("🔍 Query intent: {:?}, Context tier: {:?}", query_intent, context_tier);
            ...
            if let Ok(memories) = memory_system.read().await.retrieve(&query) {
                if !memories.is_empty() {
                    ...
                    for (i, memory) in memories.iter().take(3).enumerate() {
                        memory_context.push_str(&format!("{}. {}\n", i + 1, memory.experience.content));
                    }
                    tracing::info!("💡 Retrieved {} relevant memories ({} chars)", memories.len(), memory_context.len());
                }
            }
        }
    ...
    tracing::info!("🔧 System context: {} chars, Memory context: {} chars",
        system_context.len(), memory_context.len());
    ...
    tracing::info!("🎯 Intent-based max_tokens: {} (for {:?})", max_tokens, query_intent);

    let response = manager.generate_custom(&enhanced_prompt, max_tokens).await.map_err(|e| e.to_string())?;

    let duration_ms = start_time.elapsed().as_millis() as u64;
    let input_tokens = estimate_tokens(&enhanced_prompt);
    let output_tokens = estimate_tokens(&response);

    tracing::info!("📥 LLM response: {} chars, {} tokens in {:.2}s ({:.1} tok/s)",
        response.len(), output_tokens, duration_ms as f64 / 1000.0,
...
pub async fn llm_generate_stream(
    ...
    // Get stream
    let mut stream = manager.generate_stream(&prompt).await.map_err(|e| e.to_string())?;
    
    // Spawn task to emit tokens
    let stream_id_clone = stream_id.clone();
    tokio::spawn(async move {
        while let Some(token) = stream.next().await {
            let _ = app_handle.emit("llm-token", &StreamToken {
...
pub async fn llm_generate_stream_with_rag(
    ...
    if let Some(ref conv_mgr) = *conv_mgr_guard {
        if let Ok(Some(conversation)) = conv_mgr.get_last_conversation().await {
            let recent_messages: Vec<String> = conversation.messages.iter()
                .rev()
            ...
            if !recent_messages.is_empty() {
                full_context_parts.push(format!("## Recent Conversation\n{}", recent_messages.join("\n")));
    ...
    match memory_result {
        Ok(Some(memories)) if !memories.is_empty() => {
            let memory_context: Vec<String> = memories.iter()
                .map(|m| format!("Memory (importance: {:.2}): {}", m.importance, m.experience.content))
                .collect();
            full_context_parts.push(format!("## Relevant Memories\n{}", memory_context.join("\n")));
        ...
        Err(_) => {
            tracing::info!("    ⚠ Memory retrieval timed out after 3s, continuing without memories");
        }
    }
    ...
    if !context.is_empty() {
        ...
        let compressed: Vec<String> = context.iter()
            .map(|chunk| shodh_rag::rag::context_compressor::compress_chunk(chunk, &query, max_sentences))
            .filter(|c| !c.is_empty())
            .collect();
        ...
        full_context_parts.push(format!("## Retrieved Documents\n{}", compressed.join("\n\n")));
        tracing::info!("    ✓ Added {} RAG results (compressed {}% : {} → {} chars)",
            compressed.len(), reduction, original_chars, compressed_chars);
    }
    ...
    let model_name = manager_info.map(|info| info.model).unwrap_or_else(|| "llm".to_string()); // Get model name before moving into closure
    ...
    tokio::spawn(async move {
        ...
        tracing::info!("     📊 Stream complete: {} input tokens, {} output tokens, {:.2}s total",
            input_tokens_for_spawn, output_token_count, total_duration);
        ...
        let _ = app_handle.emit("llm-metadata", &serde_json::json!({
            "stream_id": stream_id_clone,
            "model": model_name,
...
pub async fn get_model_cache_info(
    ...
    let cached_models = state.model_manager.list_cached_models().await
        .map_err(|e| e.to_string())?;
    
    let cache_size = state.model_manager.get_cache_size().await
        .map_err(|e| e.to_string())?;
...
pub async fn delete_cached_model(
    ...
    state.model_manager.delete_model(&model_enum).await
        .map_err(|e| e.to_string())
}
