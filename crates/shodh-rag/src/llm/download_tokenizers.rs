...
pub async fn download_all_tokenizers(cache_dir: &Path) -> Result<()> {
    println!("Downloading tokenizers to {:?}", cache_dir);
    ...
    for (filename, url) in TOKENIZER_URLS {
        ...
        if file_path.exists() {
            println!("✓ Tokenizer {} already exists", filename);
            success_count += 1;
            continue;
        }
            ...
            match download_file(&client, url, &file_path).await {
                ...
                Err(e) => {
                    ...
                    if retry_count < max_retries {
                        ...
                        println!("  Retry {}/{} after {:?}...", retry_count, max_retries, wait_time);
        ...
        if retry_count >= max_retries {
            eprintln!("✗ Failed to download {}: {}", filename, last_error.unwrap_or_default());
    ...
    println!("  Successful: {}/{}", success_count, TOKENIZER_URLS.len());
    ...
    if !failed_downloads.is_empty() {
        ...
        println!("\nNote: Some tokenizers require authentication.");
        println!("The system will use fallback tokenizers for missing files.");
    }
...
pub async fn download_model_tokenizer(model: &str, cache_dir: &Path) -> Result<()> {
    ...
    if !json_path.exists() {
        println!("Downloading {}", json_file);
        download_file(&client, json_url, &json_path).await?;
    }
    
    if !config_path.exists() {
        println!("Downloading {}", config_file);
