...
use ndarray::{Array4, ArrayView4, ArrayViewD, Axis, IxDyn};
...
impl GQACache {
    ...
    pub fn clear(&mut self) {
        ...
        for layer_idx in 0..self.config.num_layers {
            self.key_cache[layer_idx] =
                Array4::zeros((1, self.config.num_kv_heads, 1, self.config.head_dim));
    ...
    pub fn as_ndarray_inputs(&self) -> Vec<(&'static str, ArrayViewD<f32>)> {
        ...
        for layer_idx in 0..self.config.num_layers {
            ...
            inputs.push((
                value_name_static,
                self.value_cache[layer_idx].view().into_dyn(),
            ));
        }
    ...
    pub fn memory_usage(&self) -> usize {
        let elements_per_layer =
            self.config.num_kv_heads * self.sequence_length * self.config.head_dim;
