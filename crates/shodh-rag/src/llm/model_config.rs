...
impl ModelPathConfig {
    ...
    pub fn add_model(&mut self, name: String, model_file: PathBuf, data_files: Vec<PathBuf>) {
        self.model_paths.insert(name, ModelPath {
            model_file,
            data_files,
            tokenizer_path: None,
        });
    ...
    pub fn create_example() -> Self {
        ...
        config.add_model(
            ...
            vec![PathBuf::from("models/phi3-mini-4k-instruct-cpu-int4-rtn-block-32-acc-level-4.onnx.data")]
        );
...
static MODEL_CONFIG: std::sync::OnceLock<std::sync::Arc<parking_lot::RwLock<ModelPathConfig>>> = std::sync::OnceLock::new();
...
pub fn update_model_path(model_name: String, model_file: PathBuf, data_files: Vec<PathBuf>) -> Result<()> {
